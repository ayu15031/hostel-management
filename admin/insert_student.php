	...
	if(isset($_POST['submit'])){
	    ...
	    if($num==0){
			$error="Incorrect roll no.";
				...
				$q="select * from ".$table." where book_status = '0' and roll_no='$roll'" ;
				$result = mysqli_query($con, $q);
				$row = mysqli_fetch_assoc($result);
				$num = mysqli_num_rows($result);
				...
				if($num==0){
					$error="Seat for the student already booked";
				}
				else if($row1['book_status']==1){
					$error="Seat filled";
				}
				else{
					$q="update ".$table." set seat_no='$seat', room_no='$room', book_status='1' where book_status = '0' and roll_no='$roll'" ;
					mysqli_query($con, $q);
